...
if ($help -eq $true) {
    if ($excludeHeader -eq $false) {
        echo "`"Start-Logging`" - Logs Until You Fall using `"adb logcat`""
        echo "`n-- Arguments --`n"
    }
...
$bspid = adb shell pidof com.schellgames.untilyoufall
...
if ($self -eq $true) {
    $command += " | Select-String -pattern `"(givearmaments|AndroidRuntime|CRASH)`""
}
elseif ($custom -ne "") {
